...
export class EmailSender {
  async sendNewsletter(options: SendOptions): Promise<void> {
    ...
    try {
      ...
      // 3. Notify start
      await this.notifyStart(newsletter, options.testMode);
  ...
  private async getNewsletter(newsletterId?: string): Promise<Newsletter | null> {
    ...
    // Get the latest unpublished newsletter
  ...
  private async getLatestUnsentNewsletter(): Promise<Newsletter | null> {
    // This would query for newsletters with status 'published' but not 'sent'
    // For now, we'll get the most recent newsletter
